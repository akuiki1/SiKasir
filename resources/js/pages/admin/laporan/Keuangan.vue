...
// ---------------------------------------------------------------
const mdrRates = ref<Record<string, number>>({ cash: 0, qris: 0.7, transfer: 0 });
...
const reconciliationRows = computed(() =>
    props.reconciliation.methods.map((m) => {
        ...
        return { ...m, rate, fee, net, share: props.reconciliation.total > 0 ? (m.total / props.reconciliation.total) * 100 : 0 };
    }),
);
...
function downloadCsv(): void {
    ...
    const rows: string[][] = [
        ...
        ...reconciliationRows.value.map((r) => [r.label, String(r.jumlah), String(r.total), String(-r.fee), String(r.net)]),
        ['Total', String(reconciliationTotals.value.jumlah), String(reconciliationTotals.value.gross), String(-reconciliationTotals.value.fee), String(reconciliationTotals.value.net)],
    ];
    ...
    <div class="flex h-full flex-1 flex-col gap-6 bg-slate-50 p-4 text-slate-950 dark:bg-zinc-950 dark:text-slate-100 sm:p-6">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
                ...
                <h1 class="mt-1 text-2xl font-bold tracking-tight md:text-3xl">Laporan Keuangan</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Laba rugi, arus kas, dan rekonsiliasi pembayaran usaha.</p>
            </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-2xl font-bold">{{ formatRupiah(pnl.total_revenue) }}</p>
                    </div>
                ...
                <p class="mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">HPP {{ formatRupiah(pnl.hpp) }}</p>
            </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-2xl font-bold">{{ formatRupiah(pnl.gross_profit) }}</p>
                    </div>
                    <div class="flex h-11 w-11 items-center justify-center rounded-md bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-300"><Scale class="h-5 w-5" /></div>
                </div>
                <p class="mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">Sebelum biaya operasional</p>
            </div>
            ...
            <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="mt-2 text-2xl font-bold">{{ formatRupiah(pnl.operational_expenses) }}</p>
                    </div>
                    <div class="flex h-11 w-11 items-center justify-center rounded-md bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300"><Receipt class="h-5 w-5" /></div>
                </div>
                <p class="mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">Gaji, sewa, listrik, dll</p>
            </div>

            <div class="rounded-lg border p-5 shadow-sm" :class="pnl.net_profit >= 0 ? 'border-emerald-200 bg-emerald-50/60 dark:border-emerald-500/30 dark:bg-emerald-500/10' : 'border-rose-200 bg-rose-50/60 dark:border-rose-500/30 dark:bg-rose-500/10'">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">Laba Bersih</p>
                        <p class="mt-2 text-2xl font-bold" :class="pnl.net_profit >= 0 ? 'text-emerald-700 dark:text-emerald-300' : 'text-rose-700 dark:text-rose-300'">{{ formatRupiah(pnl.net_profit) }}</p>
                    </div>
                    <div class="flex h-11 w-11 items-center justify-center rounded-md" :class="pnl.net_profit >= 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300' : 'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-300'"><Percent class="h-5 w-5" /></div>
                </div>
                <p class="mt-4 text-xs font-medium text-slate-500 dark:text-slate-400">Margin {{ pnl.margin }}%</p>
            </div>
        </div>
        ...
        <!-- ============ TAB: LABA RUGI ============ -->
        <div v-if="activeTab === 'laba_rugi'" class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1.1fr)]">
            <!-- Statement -->
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Pendapatan dikurangi HPP & biaya operasional.</p>

                <div class="mt-5 space-y-1 text-sm">
                    <p class="pb-1 text-xs font-bold uppercase tracking-wide text-slate-400">Pendapatan</p>
                    <div class="flex items-center justify-between py-1.5"><span class="text-slate-600 dark:text-slate-300">Penjualan Barang</span><span class="font-medium tabular-nums">{{ formatRupiah(pnl.product_revenue) }}</span></div>
                    <div class="flex items-center justify-between py-1.5"><span class="text-slate-600 dark:text-slate-300">Pendapatan Jasa (fee)</span><span class="font-medium tabular-nums">{{ formatRupiah(pnl.jasa_revenue) }}</span></div>
                    <div class="flex items-center justify-between border-t border-slate-200 py-2 font-semibold dark:border-zinc-700"><span>Total Pendapatan (Omzet)</span><span class="tabular-nums">{{ formatRupiah(pnl.total_revenue) }}</span></div>
                    <p v-if="pnl.total_diskon > 0" class="pt-0.5 text-xs text-slate-400">Sudah termasuk potongan diskon {{ formatRupiah(pnl.total_diskon) }}</p>

                    <p class="pb-1 pt-4 text-xs font-bold uppercase tracking-wide text-slate-400">Harga Pokok Penjualan</p>
                    <div class="flex items-center justify-between py-1.5"><span class="text-slate-600 dark:text-slate-300">HPP Barang Terjual</span><span class="font-medium tabular-nums text-rose-600 dark:text-rose-400">−{{ formatRupiah(pnl.hpp) }}</span></div>
                    <div class="flex items-center justify-between border-t border-slate-200 py-2 font-semibold dark:border-zinc-700"><span>Laba Kotor</span><span class="tabular-nums">{{ formatRupiah(pnl.gross_profit) }}</span></div>

                    <p class="pb-1 pt-4 text-xs font-bold uppercase tracking-wide text-slate-400">Biaya Operasional</p>
                    <div v-for="e in pnl.expense_breakdown" :key="e.tipe" class="flex items-center justify-between py-1.5">
                        <span class="text-slate-600 dark:text-slate-300">{{ e.label }}</span>
                        <span class="font-medium tabular-nums text-rose-600 dark:text-rose-400">−{{ formatRupiah(e.nominal) }}</span>
                    </div>
                    <p v-if="pnl.expense_breakdown.length === 0" class="py-1.5 text-sm text-slate-400">Belum ada biaya operasional pada periode ini.</p>
                    <div class="flex items-center justify-between border-t border-slate-200 py-2 font-semibold dark:border-zinc-700"><span>Total Biaya Operasional</span><span class="tabular-nums text-rose-600 dark:text-rose-400">−{{ formatRupiah(pnl.operational_expenses) }}</span></div>

                    <div class="mt-2 flex items-center justify-between rounded-lg px-3 py-3 text-base font-bold" :class="pnl.net_profit >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300'">
                        <span>Laba Bersih</span><span class="tabular-nums">{{ formatRupiah(pnl.net_profit) }}</span>
                    </div>
                </div>
            </section>
            ...
            <section class="flex flex-col gap-4">
                <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                    <div class="flex items-center justify-between gap-3 border-b border-slate-200 pb-3 dark:border-zinc-800">
                        ...
                        <div class="flex flex-wrap items-center gap-3 text-[11px] font-semibold text-slate-500 dark:text-slate-400">
                            <span class="inline-flex items-center gap-1.5"><span class="h-2.5 w-2.5 rounded-sm bg-emerald-500"></span>Pemasukan</span>
                            <span class="inline-flex items-center gap-1.5"><span class="h-2.5 w-2.5 rounded-sm bg-rose-400"></span>Pengurang</span>
                            <span class="inline-flex items-center gap-1.5"><span class="h-2.5 w-2.5 rounded-sm bg-sky-500"></span>Subtotal</span>
                        </div>
                    </div>

                    <div v-if="monthly_cost_warning" class="mt-4 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-3 text-xs text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                        <AlertTriangle class="mt-0.5 h-4 w-4 shrink-0" />
                        <p>Rentang pendek ({{ period_days }} hari) memuat biaya bulanan (gaji/sewa/pajak) — laba bisa terlihat "rugi semu".</p>
                    </div>
                            ...
                            <div class="relative h-56">
                                ...
                                <div class="flex h-full items-stretch gap-1.5">
                                    <div v-for="bar in waterfallView.bars" :key="bar.label" class="relative flex-1">
                                        <div class="absolute left-1/2 w-5 -translate-x-1/2 rounded-sm transition-all duration-300" :class="bar.colorClass" :style="{ top: bar.topPct, height: bar.heightPct }"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="mt-2 flex gap-1.5">
                                <div v-for="bar in waterfallView.bars" :key="bar.label" class="min-w-0 flex-1 text-center">
                                    <p class="truncate text-[10px] font-semibold text-slate-600 dark:text-slate-300">{{ bar.label }}</p>
                                    <p class="truncate text-[9px] font-medium text-slate-400">{{ bar.amountText }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <div v-for="card in comparisonCards" :key="card.label" class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                        <p class="text-sm font-medium text-slate-500 dark:text-slate-400">{{ card.label }}</p>
                        <p class="mt-1 text-lg font-bold" :class="card.isNegative ? 'text-rose-600 dark:text-rose-400' : ''">{{ card.valueText }}</p>
                        <p class="mt-1 inline-flex items-center gap-1 text-xs font-medium" :class="card.good === null ? 'text-slate-500 dark:text-slate-400' : card.good ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'">
                ...
                <div class="flex items-start gap-3 rounded-lg p-4 shadow-sm" :class="insight.tone === 'success' ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300'">
        ...
        <!-- ============ TAB: ARUS KAS ============ -->
        <div v-else-if="activeTab === 'arus_kas'" class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <h2 class="text-lg font-semibold">Laporan Arus Kas</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Uang tunai yang masuk dan keluar pada periode ini.</p>

                <div class="mt-5 space-y-1 text-sm">
                    <p class="pb-1 text-xs font-bold uppercase tracking-wide text-emerald-500">Kas Masuk</p>
                    <div class="flex items-center justify-between py-1.5"><span class="text-slate-600 dark:text-slate-300">Penjualan (omzet)</span><span class="font-medium tabular-nums text-emerald-600 dark:text-emerald-400">+{{ formatRupiah(cashflow.kas_masuk) }}</span></div>
                    <div class="flex items-center justify-between border-t border-slate-200 py-2 font-semibold dark:border-zinc-700"><span>Total Kas Masuk</span><span class="tabular-nums">{{ formatRupiah(cashflow.kas_masuk) }}</span></div>

                    <p class="pb-1 pt-4 text-xs font-bold uppercase tracking-wide text-rose-500">Kas Keluar</p>
                    <div class="flex items-center justify-between py-1.5">
                        <span class="text-slate-600 dark:text-slate-300">Pembelian Bahan & Produksi</span>
                        <span class="font-medium tabular-nums text-rose-600 dark:text-rose-400">−{{ formatRupiah(cashflow.pembelian_produksi) }}</span>
                    </div>
                    <div class="flex items-center justify-between py-1.5"><span class="text-slate-600 dark:text-slate-300">Biaya Operasional</span><span class="font-medium tabular-nums text-rose-600 dark:text-rose-400">−{{ formatRupiah(cashflow.biaya_operasional) }}</span></div>
                    <div class="flex items-center justify-between border-t border-slate-200 py-2 font-semibold dark:border-zinc-700"><span>Total Kas Keluar</span><span class="tabular-nums text-rose-600 dark:text-rose-400">−{{ formatRupiah(cashflow.kas_keluar) }}</span></div>

                    <div class="mt-2 flex items-center justify-between rounded-lg px-3 py-3 text-base font-bold" :class="cashflow.net_cash >= 0 ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300'">
                        <span>Arus Kas Bersih</span><span class="tabular-nums">{{ formatRupiah(cashflow.net_cash) }}</span>
                    </div>
                </div>
            </section>
            ...
            <section class="flex flex-col gap-4">
                <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                    <h3 class="text-sm font-semibold text-slate-600 dark:text-slate-300">Rincian Kas Keluar</h3>
                    <div class="mt-4 space-y-3 text-sm">
                        <div>
                            <div class="flex items-center justify-between"><span class="text-slate-600 dark:text-slate-300">Batch Produksi</span><span class="font-medium tabular-nums">{{ formatRupiah(cashflow.biaya_produksi) }}</span></div>
                            <div class="mt-1 flex items-center justify-between"><span class="text-slate-600 dark:text-slate-300">Belanja Bahan / Kemasan</span><span class="font-medium tabular-nums">{{ formatRupiah(cashflow.belanja_bahan) }}</span></div>
                            <div class="mt-1 flex items-center justify-between"><span class="text-slate-600 dark:text-slate-300">Biaya Operasional</span><span class="font-medium tabular-nums">{{ formatRupiah(cashflow.biaya_operasional) }}</span></div>
                        </div>
                    </div>
                </div>

                <div v-if="cashflow.jasa_pass_through > 0" class="flex items-start gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm dark:border-zinc-800 dark:bg-zinc-900/60">
                    <Banknote class="mt-0.5 h-5 w-5 shrink-0 text-slate-400" />
                    <p class="text-slate-600 dark:text-slate-300">
                        Titipan jasa (transfer/tarik tunai) sebesar <span class="font-semibold">{{ formatRupiah(cashflow.jasa_pass_through) }}</span> masuk lalu keluar lagi (net nol), jadi tidak dihitung dalam arus kas usaha.
                    </p>
                </div>
                ...
                <div class="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-xs text-amber-700 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-300">
                    <AlertTriangle class="mt-0.5 h-4 w-4 shrink-0" />
                    <p>Arus kas dihitung dari data yang tercatat: penjualan, biaya produksi, belanja bahan, dan pengeluaran. Pembelian stok barang "beli" yang tidak dicatat sebagai pengeluaran, setoran modal, atau pembayaran utang supplier belum termasuk.</p>
                </div>
            </section>
        </div>

        <!-- ============ TAB: REKONSILIASI ============ -->
        <div v-else class="grid gap-4 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
            <section class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                <h2 class="text-lg font-semibold">Rekonsiliasi Pembayaran</h2>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Pemasukan per metode bayar dikurangi biaya admin (MDR). Ubah tarif sesuai kesepakatan dengan penyedia.</p>
                    ...
                    <table class="w-full text-left text-sm">
                        <thead>
                            <tr class="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-500 dark:border-zinc-800 dark:text-slate-400">
                                <th class="pb-2 pr-3 font-semibold">Metode</th>
                                <th class="px-2 pb-2 text-right font-semibold">Trx</th>
                                <th class="px-2 pb-2 text-right font-semibold">Bruto</th>
                                <th class="px-2 pb-2 text-right font-semibold">MDR %</th>
                                <th class="px-2 pb-2 text-right font-semibold">Biaya</th>
                                <th class="pb-2 pl-2 text-right font-semibold">Bersih</th>
                            </tr>
                        </thead>
                            ...
                            <tr v-for="r in reconciliationRows" :key="r.metode">
                                <td class="py-3 pr-3">
                                    <span class="inline-flex items-center gap-2 font-semibold">
                                        <span class="h-2.5 w-2.5 rounded-full" :class="methodColor[r.metode]"></span>{{ r.label }}
                                    </span>
                                </td>
                                <td class="px-2 text-right text-slate-600 dark:text-slate-300">{{ r.jumlah }}</td>
                                <td class="px-2 text-right font-medium tabular-nums">{{ formatRupiah(r.total) }}</td>
                                ...
                                <td class="px-2 text-right tabular-nums text-rose-600 dark:text-rose-400">{{ r.fee > 0 ? `−${formatRupiah(r.fee)}` : '—' }}</td>
                                <td class="pl-2 text-right font-semibold tabular-nums">{{ formatRupiah(r.net) }}</td>
                            </tr>
                        </tbody>
                        <tfoot>
                            <tr class="border-t-2 border-slate-300 font-bold dark:border-zinc-600">
                                <td class="py-3 pr-3">Total</td>
                                <td class="px-2 text-right">{{ reconciliationTotals.jumlah }}</td>
                                <td class="px-2 text-right tabular-nums">{{ formatRupiah(reconciliationTotals.gross) }}</td>
                                <td class="px-2"></td>
                                <td class="px-2 text-right tabular-nums text-rose-600 dark:text-rose-400">{{ reconciliationTotals.fee > 0 ? `−${formatRupiah(reconciliationTotals.fee)}` : '—' }}</td>
                                <td class="pl-2 text-right tabular-nums">{{ formatRupiah(reconciliationTotals.net) }}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>

                <p class="mt-4 text-xs text-slate-400">QRIS &amp; Transfer masuk ke rekening bank (setelah dipotong MDR); Tunai tetap berupa uang fisik di laci.</p>
            </section>

            <section class="flex flex-col gap-4">
                <div class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                    <h3 class="text-sm font-semibold text-slate-600 dark:text-slate-300">Komposisi Pembayaran</h3>
                    <div class="mt-4 space-y-4">
                        <div v-for="r in reconciliationRows" :key="r.metode">
                            <div class="flex items-center justify-between text-sm">
                                <span class="font-semibold">{{ r.label }}</span>
                                <span class="text-slate-500 dark:text-slate-400">{{ formatRupiah(r.total) }} · {{ Math.round(r.share) }}%</span>
                            </div>
                            <div class="mt-2 h-2.5 rounded-full bg-slate-100 dark:bg-zinc-800">
                                <div class="h-full rounded-full transition-all" :class="methodColor[r.metode]" :style="{ width: `${r.share}%` }"></div>
                            </div>
                        </div>
                        <p v-if="reconciliationTotals.gross === 0" class="text-sm text-slate-400">Belum ada transaksi pada periode ini.</p>
                    </div>
                </div>

                <div class="rounded-lg border border-sky-200 bg-sky-50 p-5 dark:border-sky-500/30 dark:bg-sky-500/10">
                    <p class="text-sm font-medium text-sky-700 dark:text-sky-300">Estimasi masuk rekening bank</p>
                    <p class="mt-1 text-2xl font-bold text-sky-700 dark:text-sky-300">
                        {{ formatRupiah(reconciliationRows.filter((r) => r.metode !== 'cash').reduce((s, r) => s + r.net, 0)) }}
                    </p>
                    <p class="mt-1 text-xs text-sky-600/80 dark:text-sky-400/80">Dari QRIS &amp; Transfer, setelah dipotong biaya admin.</p>
                </div>
            </section>
        </div>
            ...
            <div class="mt-5 overflow-x-auto">
                <div class="flex h-48 min-w-[640px] items-end gap-1.5 border-b border-slate-200 px-1 pb-7 dark:border-zinc-800">
                    <div v-for="item in revenueColumns" :key="item.label" class="group relative flex h-full flex-1 flex-col items-center justify-end">
                        <div class="absolute bottom-[calc(100%+0.4rem)] hidden whitespace-nowrap rounded-md border border-slate-200 bg-white px-2 py-1 text-xs shadow-lg group-hover:block dark:border-zinc-700 dark:bg-zinc-950">
                            <span class="font-semibold">{{ item.label }}</span> · {{ formatRupiah(item.value) }}
                        </div>
