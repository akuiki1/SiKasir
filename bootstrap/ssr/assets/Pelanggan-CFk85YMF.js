...
import { Fragment, computed, createBlock, createCommentVNode, createTextVNode, createVNode, defineComponent, openBlock, ref, renderList, toDisplayString, unref, useSSRContext, vModelText, withCtx, withDirectives, withModifiers } from "vue";
...
var Pelanggan_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		function submitForm() {
			if (editing.value) router.put(update(editing.value.id_pelanggan).url, form.data(), {
			...
			_push(ssrRenderComponent(BodyTeleport_default, null, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) if (showModal.value) {
						_push(`<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm"${_scopeId}><div class="w-full max-w-md rounded-2xl border border-sidebar-border/70 bg-card p-6 shadow-2xl dark:border-sidebar-border"${_scopeId}><div class="mb-5 flex items-center justify-between"${_scopeId}><h2 class="text-lg font-bold"${_scopeId}>${ssrInterpolate(editing.value ? "Edit Pelanggan" : "Tambah Pelanggan")}</h2><button class="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-slate-100 dark:hover:bg-zinc-800"${_scopeId}>`);
						_push(ssrRenderComponent(unref(X), { class: "h-5 w-5" }, null, _parent, _scopeId));
						_push(`</button></div><form class="flex flex-col gap-4"${_scopeId}><div${_scopeId}><label class="mb-1.5 block text-sm font-medium" for="pel-nama"${_scopeId}>Nama</label><input id="pel-nama"${ssrRenderAttr("value", unref(form).nama)} type="text" placeholder="Nama pelanggan" class="${ssrRenderClass([{ "border-rose-500": unref(form).errors.nama }, "w-full rounded-lg border border-sidebar-border/70 bg-background px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"])}"${_scopeId}>`);
						...
						_push(`</div><div${_scopeId}><label class="mb-1.5 block text-sm font-medium" for="pel-telp"${_scopeId}>Telepon (opsional)</label><input id="pel-telp"${ssrRenderAttr("value", unref(form).telp)} type="text" inputmode="tel" placeholder="08xxxxxxxxxx" class="${ssrRenderClass([{ "border-rose-500": unref(form).errors.telp }, "w-full rounded-lg border border-sidebar-border/70 bg-background px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"])}"${_scopeId}>`);
						if (unref(form).errors.telp) {
							_push(`<p class="mt-1 flex items-center gap-1 text-xs text-rose-600"${_scopeId}>`);
							_push(ssrRenderComponent(unref(AlertCircle), { class: "h-3 w-3" }, null, _parent, _scopeId));
							_push(`${ssrInterpolate(unref(form).errors.telp)}</p>`);
						} else _push(`<!---->`);
						_push(`</div><div${_scopeId}><label class="mb-1.5 block text-sm font-medium"${_scopeId}>Tipe Pelanggan</label><div class="grid grid-cols-2 gap-2"${_scopeId}><!--[-->`);
						ssrRenderList(["umum", "reseller"], (opt) => {
							_push(`<button type="button" class="${ssrRenderClass(["rounded-lg border px-3 py-2 text-sm font-semibold capitalize transition", unref(form).tipe === opt ? "border-indigo-500 bg-indigo-600 text-white shadow-sm" : "border-sidebar-border/70 bg-background text-muted-foreground hover:bg-slate-50 dark:border-sidebar-border dark:hover:bg-zinc-800"])}"${_scopeId}>${ssrInterpolate(opt)}</button>`);
						});
						_push(`<!--]--></div><p class="mt-1 text-xs text-muted-foreground"${_scopeId}> Reseller mendapat potongan harga per produk (diatur di Data Produk) saat dipilih di kasir. </p></div><div class="flex justify-end gap-3 pt-2"${_scopeId}><button type="button" class="rounded-lg border border-sidebar-border/70 bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-slate-50 dark:border-sidebar-border dark:hover:bg-zinc-800/40"${_scopeId}> Batal </button><button type="submit" class="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-500 disabled:opacity-60"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""}${_scopeId}>`);
					...
					}, [createVNode("div", { class: "w-full max-w-md rounded-2xl border border-sidebar-border/70 bg-card p-6 shadow-2xl dark:border-sidebar-border" }, [createVNode("div", { class: "mb-5 flex items-center justify-between" }, [createVNode("h2", { class: "text-lg font-bold" }, toDisplayString(editing.value ? "Edit Pelanggan" : "Tambah Pelanggan"), 1), createVNode("button", {
						...
						createVNode("div", null, [
							...
							withDirectives(createVNode("input", {
								...
								class: ["w-full rounded-lg border border-sidebar-border/70 bg-background px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border", { "border-rose-500": unref(form).errors.telp }]
							}, null, 10, ["onUpdate:modelValue"]), [[vModelText, unref(form).telp]]),
							unref(form).errors.telp ? (openBlock(), createBlock("p", {
								key: 0,
								class: "mt-1 flex items-center gap-1 text-xs text-rose-600"
							}, [createVNode(unref(AlertCircle), { class: "h-3 w-3" }), createTextVNode(toDisplayString(unref(form).errors.telp), 1)])) : createCommentVNode("", true)
						]),
						createVNode("div", null, [
							createVNode("label", { class: "mb-1.5 block text-sm font-medium" }, "Tipe Pelanggan"),
							createVNode("div", { class: "grid grid-cols-2 gap-2" }, [(openBlock(), createBlock(Fragment, null, renderList(["umum", "reseller"], (opt) => {
								return createVNode("button", {
									key: opt,
									type: "button",
									class: ["rounded-lg border px-3 py-2 text-sm font-semibold capitalize transition", unref(form).tipe === opt ? "border-indigo-500 bg-indigo-600 text-white shadow-sm" : "border-sidebar-border/70 bg-background text-muted-foreground hover:bg-slate-50 dark:border-sidebar-border dark:hover:bg-zinc-800"],
									onClick: ($event) => unref(form).tipe = opt
								}, toDisplayString(opt), 11, ["onClick"]);
							}), 64))]),
							createVNode("p", { class: "mt-1 text-xs text-muted-foreground" }, " Reseller mendapat potongan harga per produk (diatur di Data Produk) saat dipilih di kasir. ")
						]),
						...
						}, [createVNode(unref(Save), { class: "h-4 w-4" }), createTextVNode(" " + toDisplayString(editing.value ? "Simpan Perubahan" : "Tambah Pelanggan"), 1)], 8, ["disabled"])])
...
//# sourceMappingURL=Pelanggan-CFk85YMF.js.map