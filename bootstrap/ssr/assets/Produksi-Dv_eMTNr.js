	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			ssrRenderTeleport(_push, (_push) => {
				if (showModal.value) {
					_push(`<div class="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-0 sm:items-center sm:p-4"><div class="flex max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-3xl border border-sidebar-border/70 bg-card shadow-2xl sm:rounded-3xl dark:border-sidebar-border"><div class="flex items-start justify-between gap-4 border-b border-sidebar-border/70 p-5 sm:p-6 dark:border-sidebar-border"><div><h2 class="text-lg font-semibold sm:text-xl">Catat Batch Produksi</h2><p class="mt-1 text-sm text-muted-foreground"> Pilih produk, isi jumlah hasil &amp; biaya — modal per unit dihitung otomatis. </p></div><button class="-mr-1 -mt-1 shrink-0 rounded-full p-2 text-muted-foreground transition hover:bg-slate-100 dark:hover:bg-zinc-800">`);
					_push(ssrRenderComponent(unref(X), { class: "h-5 w-5" }, null, _parent));
					_push(`</button></div><div class="flex flex-col gap-5 overflow-y-auto p-5 sm:p-6">`);
					if (__props.produks.length === 0) _push(`<div class="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-400"> Belum ada produk berjenis <strong>produksi</strong>. Buat dulu produk dengan jenis &quot;Buatan Sendiri&quot; di menu Data Produk. </div>`);
					else _push(`<!---->`);
					_push(`<div><label class="mb-2 block text-sm font-medium">Produk (Buatan Sendiri)</label>`);
					if (selectedProduk.value) {
						...
						_push(`</div><div class="min-w-0"><p class="truncate font-semibold">${ssrInterpolate(selectedProduk.value.nama)}</p><p class="text-xs text-muted-foreground"> Stok ${ssrInterpolate(formatStok(selectedProduk.value.stok))} · Modal kini ${ssrInterpolate(rupiah(selectedProduk.value.harga_modal))}</p></div></div><button type="button" class="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-xs font-semibold text-muted-foreground transition hover:bg-slate-100 dark:border-sidebar-border dark:hover:bg-zinc-800">`);
						...
						_push(ssrRenderComponent(unref(Search), { class: "absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" }, null, _parent));
						_push(`<input${ssrRenderAttr("value", produkSearch.value)} type="text" placeholder="Ketik nama produk untuk mencari..." class="w-full rounded-lg border border-sidebar-border/70 bg-background py-2.5 pl-9 pr-4 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"></div>`);
						if (!produkQuery.value) _push(`<p class="mt-2 px-1 text-xs text-muted-foreground"> Mulai ketik nama produk untuk menampilkan daftarnya. </p>`);
							...
							ssrRenderList(filteredProdukOptions.value, (p) => {
								_push(`<button type="button" class="flex w-full items-center justify-between gap-3 rounded-lg border border-transparent px-3 py-2.5 text-left transition-colors hover:border-indigo-500/30 hover:bg-indigo-500/5"><span class="min-w-0 truncate text-sm font-medium">${ssrInterpolate(p.nama)}</span><span class="shrink-0 rounded-md bg-muted px-2 py-0.5 text-[11px] font-medium tabular-nums text-muted-foreground"> Stok ${ssrInterpolate(formatStok(p.stok))}</span></button>`);
							});
							_push(`<!--]--></div>`);
						}
						_push(`</div>`);
					} else _push(`<!---->`);
					if (unref(form).errors.id_produk) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.id_produk)}</p>`);
					else _push(`<!---->`);
					_push(`</div><div><label class="mb-2 block text-sm font-medium">Jumlah Hasil (unit)</label><input${ssrRenderAttr("value", unref(form).jumlah)} type="number" min="1" inputmode="numeric" placeholder="0" class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border">`);
					if (unref(form).errors.jumlah) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.jumlah)}</p>`);
					else _push(`<!---->`);
					_push(`</div><div><div class="mb-2 flex items-center justify-between gap-2"><label class="text-sm font-medium">Biaya Produksi</label><div class="inline-flex gap-1 rounded-lg border border-sidebar-border/70 bg-background p-1 dark:border-sidebar-border"><button type="button" class="${ssrRenderClass(["rounded-md px-3 py-1 text-xs font-semibold transition", unref(form).mode === "sederhana" ? "bg-indigo-600 text-white shadow-sm" : "text-muted-foreground hover:bg-slate-100 dark:hover:bg-zinc-800"])}"> Total </button><button type="button" class="${ssrRenderClass(["rounded-md px-3 py-1 text-xs font-semibold transition", unref(form).mode === "rinci" ? "bg-indigo-600 text-white shadow-sm" : "text-muted-foreground hover:bg-slate-100 dark:hover:bg-zinc-800"])}"> Rincian </button></div></div>`);
					if (unref(form).mode === "sederhana") {
						_push(`<div><div class="relative"><span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-muted-foreground">Rp</span><input${ssrRenderAttr("value", unref(form).total_biaya)} type="number" min="0" inputmode="numeric" placeholder="Total uang yang keluar untuk batch ini" class="w-full rounded-lg border border-sidebar-border/70 bg-background py-2.5 pl-10 pr-3 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"></div><p class="mt-1.5 text-xs text-muted-foreground">Cukup isi total biaya — modal per unit dihitung otomatis.</p>`);
						if (unref(form).errors.total_biaya) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.total_biaya)}</p>`);
						else _push(`<!---->`);
						_push(`</div>`);
					} else {
						_push(`<div><div class="space-y-2"><!--[-->`);
						ssrRenderList(unref(form).biayas, (biaya, index) => {
							_push(`<div class="flex items-center gap-2"><input${ssrRenderAttr("value", biaya.nama)} type="text" placeholder="Nama bahan, mis. bawang ~2kg" class="min-w-0 flex-1 rounded-lg border border-sidebar-border/70 bg-background px-3 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"><div class="relative w-28 shrink-0 sm:w-36"><span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-muted-foreground">Rp</span><input${ssrRenderAttr("value", biaya.nominal)} type="number" min="0" inputmode="numeric" placeholder="0" class="w-full rounded-lg border border-sidebar-border/70 bg-background py-2.5 pl-9 pr-2 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"></div><button type="button" class="shrink-0 rounded-lg p-2 text-muted-foreground transition hover:bg-slate-100 hover:text-rose-600 disabled:opacity-30 dark:hover:bg-zinc-800"${ssrIncludeBooleanAttr(unref(form).biayas.length <= 1) ? " disabled" : ""}>`);
							_push(ssrRenderComponent(unref(Trash2), { class: "h-4 w-4" }, null, _parent));
							_push(`</button></div>`);
						});
						_push(`<!--]--></div><button type="button" class="mt-2 inline-flex items-center gap-1.5 text-xs font-semibold text-indigo-600 hover:text-indigo-500 dark:text-indigo-400">`);
						_push(ssrRenderComponent(unref(PlusCircle), { class: "h-4 w-4" }, null, _parent));
						_push(` Tambah baris bahan </button><p class="mt-1.5 text-xs text-muted-foreground">Rinci biaya bahan yang <em>terpakai</em> (bukan total pembelian).</p>`);
						if (unref(form).errors.biayas) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.biayas)}</p>`);
						else _push(`<!---->`);
						if (unref(form).errors.total_biaya) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.total_biaya)}</p>`);
						else _push(`<!---->`);
						_push(`</div>`);
					}
					_push(`</div><div><label class="mb-2 block text-sm font-medium">Catatan (opsional)</label><textarea rows="2" placeholder="Mis. tanggal masak atau kode batch." class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border">${ssrInterpolate(unref(form).catatan)}</textarea></div><div class="grid grid-cols-3 gap-3 rounded-xl border border-indigo-500/20 bg-indigo-500/5 p-4"><div><span class="text-xs font-medium text-muted-foreground">Total Biaya</span><p class="mt-0.5 text-sm font-bold sm:text-lg">${ssrInterpolate(rupiah(totalBiaya.value))}</p></div><div><span class="text-xs font-medium text-muted-foreground">Jumlah Hasil</span><p class="mt-0.5 text-sm font-bold sm:text-lg">${ssrInterpolate(formatStok(unref(form).jumlah || 0))} unit</p></div><div><span class="text-xs font-medium text-muted-foreground">Modal / Unit</span><p class="mt-0.5 text-sm font-bold text-indigo-600 sm:text-lg dark:text-indigo-400">${ssrInterpolate(rupiah(modalPerUnit.value))}</p></div></div>`);
					if (selectedProduk.value) _push(`<p class="-mt-2 text-xs text-muted-foreground"> Modal produk akan diperbarui dari <strong>${ssrInterpolate(rupiah(selectedProduk.value.harga_modal))}</strong> ke ${ssrInterpolate(rupiah(modalPerUnit.value))} setelah disimpan. </p>`);
					else _push(`<!---->`);
					_push(`</div><div class="flex flex-col gap-3 border-t border-sidebar-border/70 p-5 sm:flex-row sm:justify-end sm:p-6 dark:border-sidebar-border"><button class="rounded-lg border border-sidebar-border/70 bg-background px-4 py-2.5 text-sm font-semibold text-muted-foreground hover:bg-slate-100 dark:border-sidebar-border dark:hover:bg-zinc-800" type="button"> Batal </button><button class="inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-indigo-500 disabled:opacity-50" type="button"${ssrIncludeBooleanAttr(unref(form).processing || __props.produks.length === 0) ? " disabled" : ""}>`);
...
//# sourceMappingURL=Produksi-Dv_eMTNr.js.map