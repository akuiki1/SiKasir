...
import { computed, createVNode, defineComponent, onBeforeUnmount, onMounted, ref, resolveDynamicComponent, unref, useSSRContext } from "vue";
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			ssrRenderTeleport(_push, (_push) => {
				if (showDetail.value && selectedTrx.value) {
					_push(`<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"><div class="w-full max-w-lg rounded-2xl border border-sidebar-border/70 bg-card p-6 shadow-2xl dark:border-sidebar-border" style="${ssrRenderStyle({
						"max-height": "90vh",
						"overflow-y": "auto"
					})}"><div class="mb-5 flex items-center justify-between"><h2 class="text-lg font-bold">Detail Transaksi</h2><button class="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-slate-100 dark:hover:bg-zinc-800">`);
					_push(ssrRenderComponent(unref(X), { class: "h-5 w-5" }, null, _parent));
					_push(`</button></div><div class="mb-4 flex flex-col gap-3 text-sm"><div class="flex justify-between"><span class="text-muted-foreground">ID Transaksi</span><span class="font-mono font-bold text-indigo-600 dark:text-indigo-400">${ssrInterpolate(selectedTrx.value.kode)}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Kasir</span><span class="font-semibold">${ssrInterpolate(selectedTrx.value.kasir)}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Waktu</span><span>${ssrInterpolate(selectedTrx.value.waktu)}, ${ssrInterpolate(selectedTrx.value.tanggal)}</span></div></div><div class="mb-4 overflow-hidden rounded-lg border border-sidebar-border/70 dark:border-sidebar-border"><table class="w-full text-sm"><thead><tr class="border-b border-sidebar-border/70 bg-slate-50/50 dark:border-sidebar-border dark:bg-zinc-800/20"><th class="px-4 py-2 text-left font-semibold text-muted-foreground">Produk</th><th class="px-4 py-2 text-right font-semibold text-muted-foreground">Qty</th><th class="px-4 py-2 text-right font-semibold text-muted-foreground">Subtotal</th></tr></thead><tbody class="divide-y divide-sidebar-border/70 dark:divide-sidebar-border"><!--[-->`);
					ssrRenderList(selectedTrx.value.details, (detail, idx) => {
						_push(`<tr><td class="px-4 py-2 flex items-center gap-3">`);
						if (detail.foto_url) _push(`<img${ssrRenderAttr("src", detail.foto_url)}${ssrRenderAttr("alt", detail.nama_produk)} class="h-10 w-10 shrink-0 rounded-lg border border-sidebar-border/70 object-cover dark:border-sidebar-border">`);
						else _push(`<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-sidebar-border/70 bg-slate-100 text-[10px] font-medium text-muted-foreground dark:border-sidebar-border dark:bg-zinc-800"> Foto </div>`);
						_push(`<span>${ssrInterpolate(detail.nama_produk)}</span></td><td class="px-4 py-2 text-right">${ssrInterpolate(detail.jumlah)}</td><td class="px-4 py-2 text-right font-medium">${ssrInterpolate(unref(formatRupiah)(detail.subtotal))}</td></tr>`);
					});
					_push(`<!--]--></tbody></table></div><div class="flex flex-col gap-2 text-sm"><div class="flex justify-between"><span class="text-muted-foreground">Metode Pembayaran</span><span class="font-semibold">${ssrInterpolate(formatMetode(selectedTrx.value.metode_pembayaran))}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Total Belanja</span><span class="font-bold">${ssrInterpolate(unref(formatRupiah)(selectedTrx.value.total_harga))}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Dibayar</span><span>${ssrInterpolate(unref(formatRupiah)(selectedTrx.value.bayar))}</span></div><div class="flex justify-between"><span class="text-muted-foreground">Kembalian</span><span class="font-semibold text-emerald-600">${ssrInterpolate(unref(formatRupiah)(selectedTrx.value.kembalian))}</span></div></div><div class="mt-5 flex justify-end gap-3"><button class="inline-flex items-center gap-2 rounded-lg border border-sidebar-border/70 bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-slate-50 dark:border-sidebar-border dark:hover:bg-zinc-800/40">`);
					_push(ssrRenderComponent(unref(Edit), { class: "h-4 w-4" }, null, _parent));
					_push(` Edit </button><button class="inline-flex items-center gap-2 rounded-lg border border-rose-500/20 bg-rose-500/10 px-4 py-2 text-sm font-medium text-rose-600 transition-colors hover:bg-rose-500/20">`);
					_push(ssrRenderComponent(unref(Trash2), { class: "h-4 w-4" }, null, _parent));
			...
			ssrRenderTeleport(_push, (_push) => {
				if (showFormModal.value) {
					_push(`<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm"><div class="w-full max-w-2xl rounded-2xl border border-sidebar-border/70 bg-card p-6 shadow-2xl dark:border-sidebar-border" style="${ssrRenderStyle({
						"max-height": "90vh",
						"overflow-y": "auto"
					})}"><div class="mb-5 flex items-center justify-between"><h2 class="text-lg font-bold">${ssrInterpolate(editingTransaksi.value ? "Edit Transaksi" : "Tambah Transaksi Baru")}</h2><button class="rounded-lg p-1.5 text-muted-foreground transition-colors hover:bg-slate-100 dark:hover:bg-zinc-800">`);
					_push(ssrRenderComponent(unref(X), { class: "h-5 w-5" }, null, _parent));
					_push(`</button></div><form class="flex flex-col gap-4"><div class="grid grid-cols-2 gap-4"><div><label class="mb-1.5 block text-sm font-medium" for="trx-kasir"> Kasir </label><select id="trx-kasir" class="${ssrRenderClass([{ "border-rose-500": unref(form).errors.id_user }, "w-full rounded-lg border border-sidebar-border/70 bg-background px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"])}"><option value=""${ssrIncludeBooleanAttr(Array.isArray(unref(form).id_user) ? ssrLooseContain(unref(form).id_user, "") : ssrLooseEqual(unref(form).id_user, "")) ? " selected" : ""}>Pilih kasir</option><!--[-->`);
					ssrRenderList(__props.kasirs, (kasir) => {
						_push(`<option${ssrRenderAttr("value", String(kasir.id))}${ssrIncludeBooleanAttr(Array.isArray(unref(form).id_user) ? ssrLooseContain(unref(form).id_user, String(kasir.id)) : ssrLooseEqual(unref(form).id_user, String(kasir.id))) ? " selected" : ""}>${ssrInterpolate(kasir.name)} (${ssrInterpolate(kasir.role)}) </option>`);
					});
					_push(`<!--]--></select>`);
					...
					_push(`</div><div><label class="mb-1.5 block text-sm font-medium" for="trx-metode"> Metode Pembayaran </label><select id="trx-metode" class="w-full rounded-lg border border-sidebar-border/70 bg-background px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"><option value="cash"${ssrIncludeBooleanAttr(Array.isArray(unref(form).metode_pembayaran) ? ssrLooseContain(unref(form).metode_pembayaran, "cash") : ssrLooseEqual(unref(form).metode_pembayaran, "cash")) ? " selected" : ""}>Tunai</option><option value="qris"${ssrIncludeBooleanAttr(Array.isArray(unref(form).metode_pembayaran) ? ssrLooseContain(unref(form).metode_pembayaran, "qris") : ssrLooseEqual(unref(form).metode_pembayaran, "qris")) ? " selected" : ""}>QRIS</option><option value="transfer"${ssrIncludeBooleanAttr(Array.isArray(unref(form).metode_pembayaran) ? ssrLooseContain(unref(form).metode_pembayaran, "transfer") : ssrLooseEqual(unref(form).metode_pembayaran, "transfer")) ? " selected" : ""}>Transfer</option></select></div></div><div><div class="mb-2 flex items-center justify-between"><label class="text-sm font-medium">Item Produk</label><button type="button" class="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-500">`);
					_push(ssrRenderComponent(unref(Plus), { class: "h-3 w-3" }, null, _parent));
					_push(` Tambah Item </button></div>`);
					if (unref(form).errors.items) {
						_push(`<div class="mb-2 flex items-center gap-1 text-xs text-rose-600">`);
						_push(ssrRenderComponent(unref(AlertCircle), { class: "h-3 w-3" }, null, _parent));
					...
					ssrRenderList(unref(form).items, (item, index) => {
						_push(`<div class="flex items-start gap-2 rounded-lg border border-sidebar-border/70 p-3 dark:border-sidebar-border"><div class="flex-1"><select class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"><option value=""${ssrIncludeBooleanAttr(Array.isArray(item.id_produk) ? ssrLooseContain(item.id_produk, "") : ssrLooseEqual(item.id_produk, "")) ? " selected" : ""}>Pilih produk</option><!--[-->`);
						ssrRenderList(__props.produks, (produk) => {
							_push(`<option${ssrRenderAttr("value", String(produk.id_produk))}${ssrIncludeBooleanAttr(Array.isArray(item.id_produk) ? ssrLooseContain(item.id_produk, String(produk.id_produk)) : ssrLooseEqual(item.id_produk, String(produk.id_produk))) ? " selected" : ""}>${ssrInterpolate(produk.nama)} — ${ssrInterpolate(unref(formatRupiah)(produk.harga_jual))} (stok: ${ssrInterpolate(produk.stok)}) </option>`);
						});
						_push(`<!--]--></select></div><div class="w-24"><input${ssrRenderAttr("value", item.jumlah)} type="number" min="1"${ssrRenderAttr("max", getProdukStok(item.id_produk) || void 0)} placeholder="Qty" class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"></div><div class="w-28 pt-2 text-right text-sm font-medium">${ssrInterpolate(item.id_produk && item.jumlah ? unref(formatRupiah)(getProdukHarga(item.id_produk) * Number(item.jumlah)) : "-")}</div><button type="button"${ssrIncludeBooleanAttr(unref(form).items.length <= 1) ? " disabled" : ""} class="${ssrRenderClass([{ "opacity-40": unref(form).items.length <= 1 }, "mt-1.5 rounded-lg p-1 text-muted-foreground transition-colors hover:bg-slate-100 hover:text-rose-600 dark:hover:bg-zinc-800"])}">`);
						_push(ssrRenderComponent(unref(Minus), { class: "h-4 w-4" }, null, _parent));
						_push(`</button></div>`);
					});
					_push(`<!--]--></div></div><div class="rounded-lg border border-sidebar-border/70 bg-slate-50/50 p-4 dark:border-sidebar-border dark:bg-zinc-800/20"><div class="flex justify-between text-sm"><span class="text-muted-foreground">Total</span><span class="font-bold">${ssrInterpolate(unref(formatRupiah)(computedTotal.value))}</span></div><div class="mt-2 flex justify-between text-sm"><span class="text-muted-foreground">Kembalian</span><span class="font-semibold text-emerald-600">${ssrInterpolate(unref(formatRupiah)(computedKembalian.value))}</span></div></div><div><label class="mb-1.5 block text-sm font-medium" for="trx-bayar"> Jumlah Bayar (Rp) </label><input id="trx-bayar"${ssrRenderAttr("value", unref(form).bayar)} type="number" min="0" placeholder="0" class="${ssrRenderClass([{ "border-rose-500": unref(form).errors.bayar }, "w-full rounded-lg border border-sidebar-border/70 bg-background px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"])}">`);
					if (unref(form).errors.bayar) {
						_push(`<p class="mt-1 flex items-center gap-1 text-xs text-rose-600">`);
						_push(ssrRenderComponent(unref(AlertCircle), { class: "h-3 w-3" }, null, _parent));
						_push(`${ssrInterpolate(unref(form).errors.bayar)}</p>`);
					} else _push(`<!---->`);
					_push(`</div><div class="flex justify-end gap-3 pt-2"><button type="button" class="rounded-lg border border-sidebar-border/70 bg-background px-4 py-2 text-sm font-medium transition-colors hover:bg-slate-50 dark:border-sidebar-border dark:hover:bg-zinc-800/40"> Batal </button><button type="submit" class="inline-flex cursor-pointer items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-indigo-500 disabled:opacity-60"${ssrIncludeBooleanAttr(unref(form).processing) ? " disabled" : ""}>`);
...
//# sourceMappingURL=Transactions-DCC648K3.js.map