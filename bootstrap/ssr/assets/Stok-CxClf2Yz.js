...
var Stok_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(`<input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Cari nama produk / kategori..." class="w-full rounded-lg border border-sidebar-border/70 bg-background py-2 pl-9 pr-4 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"></div><select class="rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border"><option value="all"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "all") : ssrLooseEqual(statusFilter.value, "all")) ? " selected" : ""}>Semua status</option><option value="in-stock"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "in-stock") : ssrLooseEqual(statusFilter.value, "in-stock")) ? " selected" : ""}>Tersedia</option><option value="low-stock"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "low-stock") : ssrLooseEqual(statusFilter.value, "low-stock")) ? " selected" : ""}>Menipis</option><option value="out-of-stock"${ssrIncludeBooleanAttr(Array.isArray(statusFilter.value) ? ssrLooseContain(statusFilter.value, "out-of-stock") : ssrLooseEqual(statusFilter.value, "out-of-stock")) ? " selected" : ""}>Habis</option></select></div><div class="overflow-x-auto"><table class="w-full border-collapse text-left text-sm"><thead><tr class="border-b border-sidebar-border/70 bg-slate-50/50 dark:border-sidebar-border dark:bg-zinc-800/20"><th class="px-6 py-4 font-semibold text-muted-foreground">No</th><th class="px-6 py-4 font-semibold text-muted-foreground">Produk</th><th class="px-6 py-4 font-semibold text-muted-foreground">Stok</th><th class="px-6 py-4 font-semibold text-muted-foreground">Status</th><th class="px-6 py-4 font-semibold text-muted-foreground">Modal</th><th class="px-6 py-4 text-right font-semibold text-muted-foreground">Aksi</th></tr></thead><tbody class="divide-y divide-sidebar-border/70 dark:divide-sidebar-border">`);
			...
			ssrRenderTeleport(_push, (_push) => {
				if (modalMode.value) {
					_push(`<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4"><div class="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-3xl border border-sidebar-border/70 bg-card p-6 shadow-2xl dark:border-sidebar-border"><div class="flex items-start justify-between gap-4"><div><h2 class="text-xl font-semibold">${ssrInterpolate(modalMeta[modalMode.value].title)}</h2><p class="mt-1 text-sm text-muted-foreground">${ssrInterpolate(modalMeta[modalMode.value].desc)}</p></div><button class="rounded-full p-2 text-muted-foreground transition hover:bg-slate-100 dark:hover:bg-zinc-800">`);
					_push(ssrRenderComponent(unref(X), { class: "h-5 w-5" }, null, _parent));
					_push(`</button></div><div class="mt-6 grid gap-4">`);
					if (__props.produks.length === 0) _push(`<div class="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-700 dark:text-amber-400"> Belum ada produk berstok. Tambahkan produk dulu di menu Data Produk. </div>`);
					else _push(`<!---->`);
					_push(`<div><label class="mb-2 block text-sm font-medium">Produk</label>`);
					...
					if (modalMode.value === "masuk") {
						_push(`<!--[--><div><label class="mb-2 block text-sm font-medium">Jumlah Masuk${ssrInterpolate(selectedProduk.value ? ` (${selectedProduk.value.satuan})` : "")}</label><input${ssrRenderAttr("value", unref(form).jumlah)} type="number" min="0" step="any" placeholder="0" class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-emerald-500 focus:outline-none dark:border-sidebar-border">`);
						...
						if (!selectedProduk.value || selectedProduk.value.jenis === "beli") {
							_push(`<div><label class="mb-2 block text-sm font-medium">Harga Beli / Modal Baru (opsional)</label><div class="relative"><span class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-sm font-semibold text-muted-foreground">Rp</span><input${ssrRenderAttr("value", unref(form).harga_beli)} type="number" min="0" placeholder="Kosongkan jika harga beli tidak berubah" class="w-full rounded-lg border border-sidebar-border/70 bg-background py-2 pl-10 pr-3 text-sm focus:border-emerald-500 focus:outline-none dark:border-sidebar-border"></div><p class="mt-1 text-xs text-muted-foreground">Isi bila harga beli dari supplier berubah — modal produk akan diperbarui.</p>`);
							if (unref(form).errors.harga_beli) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.harga_beli)}</p>`);
							else _push(`<!---->`);
							_push(`</div>`);
						} else _push(`<div class="rounded-lg border border-sky-500/30 bg-sky-500/10 p-3 text-xs text-sky-700 dark:text-sky-400"> Produk buatan sendiri — modal dikelola lewat menu <strong>Produksi</strong> (batch costing), jadi tidak diubah di sini. </div>`);
						_push(`<!--]-->`);
					} else if (modalMode.value === "keluar") {
						_push(`<!--[--><div><label class="mb-2 block text-sm font-medium">Jumlah Keluar${ssrInterpolate(selectedProduk.value ? ` (${selectedProduk.value.satuan})` : "")}</label><input${ssrRenderAttr("value", unref(form).jumlah)} type="number" min="0" step="any" placeholder="0" class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-rose-500 focus:outline-none dark:border-sidebar-border">`);
						if (unref(form).errors.jumlah) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.jumlah)}</p>`);
						else _push(`<!---->`);
						_push(`</div><div><label class="mb-2 block text-sm font-medium">Alasan</label><select class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-rose-500 focus:outline-none dark:border-sidebar-border"><!--[-->`);
						ssrRenderList(alasanOptions, (opt) => {
							_push(`<option${ssrRenderAttr("value", opt.value)}${ssrIncludeBooleanAttr(Array.isArray(unref(form).alasan) ? ssrLooseContain(unref(form).alasan, opt.value) : ssrLooseEqual(unref(form).alasan, opt.value)) ? " selected" : ""}>${ssrInterpolate(opt.label)}</option>`);
						});
						_push(`<!--]--></select>`);
						if (unref(form).errors.alasan) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.alasan)}</p>`);
						else _push(`<!---->`);
						_push(`</div><!--]-->`);
					} else {
						_push(`<!--[--><div><label class="mb-2 block text-sm font-medium">Stok Fisik (hasil hitung)${ssrInterpolate(selectedProduk.value ? ` (${selectedProduk.value.satuan})` : "")}</label><input${ssrRenderAttr("value", unref(form).stok_fisik)} type="number" min="0" step="any" placeholder="0" class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-amber-500 focus:outline-none dark:border-sidebar-border">`);
						if (unref(form).errors.stok_fisik) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.stok_fisik)}</p>`);
						else _push(`<!---->`);
						_push(`</div>`);
						if (selectedProduk.value && unref(form).stok_fisik !== null) _push(`<div class="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-sm"> Selisih penyesuaian: <strong class="${ssrRenderClass(selisihOpname.value > 0 ? "text-emerald-600 dark:text-emerald-400" : selisihOpname.value < 0 ? "text-rose-600 dark:text-rose-400" : "text-muted-foreground")}">${ssrInterpolate(selisihOpname.value > 0 ? "+" : "")}${ssrInterpolate(formatStok(selisihOpname.value, selectedProduk.value.satuan))}</strong></div>`);
						else _push(`<!---->`);
						_push(`<!--]-->`);
					}
					_push(`<div><label class="mb-2 block text-sm font-medium">Catatan (opsional)</label><input${ssrRenderAttr("value", unref(form).keterangan)} type="text" placeholder="Mis. nota #123, supplier, dsb." class="w-full rounded-lg border border-sidebar-border/70 bg-background px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none dark:border-sidebar-border">`);
					if (unref(form).errors.keterangan) _push(`<p class="mt-2 text-sm text-rose-600">${ssrInterpolate(unref(form).errors.keterangan)}</p>`);
					else _push(`<!---->`);
					_push(`</div></div><div class="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-end"><button class="rounded-lg border border-sidebar-border/70 bg-background px-4 py-2 text-sm font-semibold text-muted-foreground hover:bg-slate-100 dark:border-sidebar-border dark:hover:bg-zinc-800" type="button"> Batal </button><button class="${ssrRenderClass(["inline-flex items-center justify-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-white disabled:opacity-50", modalMode.value === "masuk" ? "bg-emerald-600 hover:bg-emerald-500" : modalMode.value === "keluar" ? "bg-rose-600 hover:bg-rose-500" : "bg-amber-600 hover:bg-amber-500"])}" type="button"${ssrIncludeBooleanAttr(unref(form).processing || __props.produks.length === 0 || unref(form).id_produk === "") ? " disabled" : ""}>`);
...
//# sourceMappingURL=Stok-CxClf2Yz.js.map